...
FAMILY_DESCRIPTOR_TOKENS = {"MINI", "PRO"}
...
# Aliases gerais de nomenclatura automotiva FIPE x PBEV.
# São aliases de termos, não regras por veículo. Servem para evitar hardcode por modelo.
TERM_ALIASES = {
    "DEFE": {"DEFENDER"}, "DEF": {"DEFENDER"},
    "DIE": {"DIESEL"}, "HIB": {"HIBRIDO"}, "HYB": {"HIBRIDO"},
    "SP": {"SPORT"}, "DYN": {"DYNAMIC"}, "DY": {"DYNAMIC"},
    "ULT": {"ULTIMATE"}, "ULTIM": {"ULTIMATE"}, "ULTD": {"ULTIMATE", "DARK"},
    "BLA": {"BLACK", "DARK"}, "DK": {"DARK"},
    "INT": {"INTENSE"}, "INTP": {"INTENSE", "PLUS"},
    "MT": {"MANUAL", "MEC"}, "MEC": {"MANUAL", "MT"},
    "AT": {"AUTO", "AUT", "AUTOMATICO"}, "AUT": {"AUTO", "AT", "AUTOMATICO"},
    "AWD": {"4X4"}, "4X4": {"AWD"},
    "CVT": {"AUTO", "AUTOMATICO"},
    "PICK": {"PICKUP"}, "PICAPE": {"PICKUP"}, "PICKUP": {"PICK"},
    "SPI": {"SPIDER", "CONVERSIVEL"}, "SPIDER": {"SPI", "CONVERSIVEL"},
    "DM": {"PHEV", "PLUGIN"}, "DMI": {"DM", "PHEV", "PLUGIN"}, "IDM": {"DM", "PHEV", "PLUGIN"},
    "TB": {"TURBO"},
}

# Prefixos que, combinados a números, formam identidade forte de modelo/motorização.
COMPOSITE_MODEL_PREFIXES = {"XC", "EX", "E", "P", "D", "T", "S", "SF", "H", "IX", "EQ", "I"}

# Tokens que são versão/acabamento ou pacote, não família comercial. Ausência não deve matar o match.
ACABAMENTO_TOKENS = {
    "PLUS", "ADVANCE", "ADVANCED", "PREMIUM", "LUXURY", "PRESTIGE", "LIMITED", "ULTRA",
    "ULTIMATE", "DARK", "BLACK", "HSE", "SE", "DYNAMIC", "CORE", "MOMENTUM", "INSCRIPTION",
    "INSC", "EXPRESS", "POLestar".upper(), "TROPHY", "ZEN", "INTENSE", "INTP", "INT",
}
...
class PbevService:
    ...
    def normalizar_texto(valor: Any) -> str:
        ...
        texto = re.sub(r"\bPICK\s+UP\b", "PICKUP", texto)
        texto = re.sub(r"\bPLUG\s+IN\b", "PLUGIN", texto)
        texto = re.sub(r"\bDM\s+I\b", "DMI", texto)
        texto = re.sub(r"\bI\s+DM\b", "IDM", texto)
        texto = re.sub(r"\bE\s+(\d{3,4})\b", r"E\1", texto)
        texto = re.sub(r"\bXC\s+(\d{2})\b", r"XC\1", texto)
        texto = re.sub(r"\bSF\s+(\d{2})\b", r"SF\1", texto)
    ...
    def _tokens(cls, valor: Any, *, remover_genericos: bool = True) -> set[str]:
        ...
        if not bruto:
            return set()
        seq = [t for t in bruto.split() if t]
        tokens = set(seq)
        ...
        # Junta códigos compostos que a FIPE/PBEV às vezes escrevem separados: XC 60, SF 90, D 300, P 250 etc.
        for i, token in enumerate(seq[:-1]):
            prox = seq[i + 1]
            if token in COMPOSITE_MODEL_PREFIXES and re.fullmatch(r"\d{1,5}[A-Z]*", prox):
                extras.add(f"{token}{prox}")
            if token == "PICK" and prox == "UP":
                extras.add("PICKUP")

        for token in list(tokens):
            extras |= set(TERM_ALIASES.get(token, set()))

            # Tokens colados de acabamento/motor: PRESTIGE1 -> PRESTIGE + 1; TB16V -> TB + 16V.
            m_colado_curto = re.fullmatch(r"([A-Z]{2,})(\d)$", token)
            if m_colado_curto:
                extras.add(m_colado_curto.group(1))
                extras.add(m_colado_curto.group(2))
            m_tb_valv = re.fullmatch(r"(TB|TURBO)(\d{1,2}V)", token)
            if m_tb_valv:
                extras.add("TB")
                extras.add("TURBO")
                extras.add(m_tb_valv.group(2))
            ...
            if m:
                ...
                if sufixo:
                    extras.add(sufixo)
                # P250FF, GL310EV, GS560EV etc. mantêm o prefixo+numero como token forte.
                if sufixo in {"FF", "EV", "DM"}:
                    extras.add(f"{prefixo}{numero}")
    ...
    @classmethod
    def _tokens_fortes_modelo(cls, texto: Any) -> set[str]:
        tokens = cls._tokens(texto)
        fortes: set[str] = set()
        for t in tokens:
            if t in ENGINE_TOKENS or t in FUEL_TOKENS or t in TRANS_TOKENS or t in GENERIC_TOKENS:
                continue
            if t in ACABAMENTO_TOKENS:
                continue
            if re.fullmatch(r"[A-Z]{1,4}\d{1,5}[A-Z]{0,3}", t):
                fortes.add(t)
            # Alguns modelos são puramente numéricos, mas só valem quando têm 3+ dígitos (2008, 208 etc.).
            elif re.fullmatch(r"\d{3,5}", t):
                fortes.add(t)
        # Não deixar derivativos redundantes brigarem entre si: se E2008 existe, 2008 é redundante, mas útil.
        return fortes

    @staticmethod
    def _sao_tokens_marca_modelo_compativeis(query_fortes: set[str], cand_fortes: set[str]) -> tuple[bool, list[str], list[str]]:
        if not query_fortes or not cand_fortes:
            return True, [], []
        inter = query_fortes & cand_fortes
        if inter:
            return True, sorted(inter), []
        # Se ambos têm tokens fortes e nenhum cruza, são modelos diferentes.
        return False, [], sorted(query_fortes - cand_fortes)
    ...
    def _detectar_combustivel_consulta(cls, consulta: dict[str, Any]) -> str:
        ...
        if tipo == "ELETRICO" or re.search(r"\b(ELETRICO|ELETRICA|BEV|EV|ELECTRIC)\b", texto):
            return "ELETRICO"
        if re.search(r"\b(DIESEL|DIE|D[0-9]{3})\b", texto):
            return "DIESEL"
        if tipo == "PHEV" or re.search(r"\b(PHEV|PLUG IN|PLUGIN|DM I|DMI|IDM|I DM|TFSI E|E HYBRID|RECHARGE|T8)\b", texto):
            return "PLUG_IN"
    ...
    @staticmethod
    def _propulsao_key(valor: Any) -> str:
        norm = PbevService.normalizar_texto(valor).replace(" ", "_")
        if not norm:
            return ""
        if "ELETRICO" in norm or norm in {"EV", "BEV"}:
            return "ELETRICO"
        if "PLUG" in norm or "PHEV" in norm or "VEHP" in norm or "DMI" in norm or "IDM" in norm:
            return "PLUG_IN"
        if "HIBR" in norm or "HEV" in norm or "MHEV" in norm:
            return "HIBRIDO"
        if "COMB" in norm:
            return "COMBUSTAO"
        return norm
    ...
    def _combustivel_compativel(req: str, cand_comb: str, cand_prop: str) -> tuple[bool, int, str]:
        ...
        cand_prop = PbevService._propulsao_key(cand_prop)
        if req == "ELETRICO":
        ...
        if req == "HIBRIDO":
            ...
            if cand_prop == "PLUG_IN":
                return True, 22, "híbrido FIPE compatível com PHEV/plugin PBEV"
            if cand_comb in {"DIESEL", "GASOLINA", "FLEX"}:
                return True, 14, "híbrido FIPE compatível com consumo combustível PBEV"
    ...
    def _modelo_core_tokens(cls, texto_modelo: Any) -> set[str]:
        ...
        core: set[str] = set()
        for token in tokens:
            if token in SOFT_BODY_TOKENS or token in FUEL_TOKENS or token in TRANS_TOKENS or token in ENGINE_TOKENS or token in GENERIC_TOKENS or token in ACABAMENTO_TOKENS:
                continue
            if re.fullmatch(r"20\d{2}", token) and token != "2008":
                continue
            if re.fullmatch(r"[0-9]", token):
    ...
    def calcular_score_match(self, registro: dict[str, Any], consulta: dict[str, Any]) -> dict[str, Any]:
        ...
        if "ZERO" in texto_ano_consulta or "32000" in str(consulta.get("ano_codigo") or "") or str(consulta.get("ano") or "").strip() == "32000":
            zero_km_contexto = True
        if zero_km_contexto and not ano_req:
            # Código FIPE 32000 significa zero km, não ano. Usa o ano corrente como referência
            # para priorizar a tabela PBEV mais recente compatível.
            ano_req = date.today().year
        ...
        # Tokens fortes de modelo/motorização. Evita tratar T1/T2/S06, HB20/HB20S, E2008/E208, XC60/XC90 etc. como ruído.
        query_fortes = self._tokens_fortes_modelo(query_all_norm)
        cand_fortes = self._tokens_fortes_modelo(cand_all_norm) | self._tokens_fortes_modelo(cand_model_norm)
        compat_fortes, fortes_inter, fortes_faltantes = self._sao_tokens_marca_modelo_compativeis(query_fortes, cand_fortes)
        if fortes_inter:
            score += min(28, 12 + 8 * len(fortes_inter))
            modelo_score = max(modelo_score, 32)
            penalidades = [p for p in penalidades if p != "família/modelo PBEV pouco compatível"]
            motivos.append("token forte de modelo compatível: " + ", ".join(fortes_inter[:4]))
        elif not compat_fortes:
            score -= 35
            penalidades.append("token forte da FIPE ausente/divergente no PBEV: " + ", ".join(fortes_faltantes[:4]))

        # Marcadores de ano-modelo comercial (MY25 etc.) não devem misturar grupos com consumo diferente.
        query_my = {t for t in query_all_tokens if re.fullmatch(r"MY\d{2}", t)}
        cand_my = {t for t in cand_all_tokens if re.fullmatch(r"MY\d{2}", t)}
        if query_my and not cand_my:
            score -= 18
            penalidades.append("marcador MY da FIPE ausente no PBEV: " + ", ".join(sorted(query_my)))
        if cand_my and not query_my:
            score -= 30
            penalidades.append("marcador MY do PBEV ausente na FIPE: " + ", ".join(sorted(cand_my)))
        if query_my and cand_my and query_my.isdisjoint(cand_my):
            score -= 35
            penalidades.append("marcador MY divergente")

        # Sufixos que mudam família/carroceria não podem empatar por alias parcial. Ex.: HB20 x HB20S.
        if "HB20" in query_all_tokens and "HB20S" not in query_all_tokens and "HB20S" in cand_all_tokens:
            score -= 30
            penalidades.append("modelo/carroceria divergente: FIPE HB20 x PBEV HB20S")
        if "HB20S" in query_all_tokens and "HB20S" not in cand_all_tokens:
            score -= 30
            penalidades.append("modelo/carroceria divergente: FIPE HB20S x PBEV HB20")
        ...
        # MINI/PRO e, condicionalmente, PLUS podem mudar a família (Dolphin Mini, Song Plus, Song Pro).
        family_query = query_all_tokens & FAMILY_DESCRIPTOR_TOKENS
        family_cand = cand_all_tokens & FAMILY_DESCRIPTOR_TOKENS
        plus_context = {"SONG", "DOLPHIN", "YUAN", "SEAL", "ORA"}
        if "PLUS" in query_all_tokens and query_all_tokens & plus_context:
            family_query.add("PLUS")
        if "PLUS" in cand_all_tokens and cand_all_tokens & plus_context:
            family_cand.add("PLUS")
        ...
        if ({"PICKUP", "BAU"} & query_all_tokens) and "VAN" in cand_all_tokens:
            score -= 35
            penalidades.append("carroceria divergente: FIPE pickup/baú x PBEV van")
        if "VAN" in query_all_tokens and "PICKUP" in cand_all_tokens:
            score -= 30
            penalidades.append("carroceria divergente: FIPE van x PBEV pickup")
        ...
        for penalidade in penalidades_tecnicas:
            ...
            if p_norm == "ACABAMENTO DIVERGENTE":
                continue
            if "TOKEN FORTE DA FIPE AUSENTE" in p_norm and ("TB16V" in p_norm or "PRESTIGE1" in p_norm):
                continue
    ...
    def montar_sugestao_consumo(self, registro: dict[str, Any]) -> dict[str, Any] | None:
        prop = self._propulsao_key(registro.get("tipo_propulsao_normalizado") or registro.get("tipo_propulsao"))
    ...
    def sugerir_consumo(self, consulta: dict[str, Any]) -> dict[str, Any]:
        ...
        tecnico_dominante_confiavel = (
            avaliacao_top.get("fuel_ok")
            and avaliacao_top.get("ano_compativel_fipe_pbev")
            and avaliacao_top.get("tecnica_suficiente_para_consumo")
            and float(avaliacao_top.get("modelo_score") or 0) >= 30
            and dominante
        ...
        high_conditions = (
            ...
            and float(avaliacao_top.get("modelo_score") or 0) >= 30
            and dominante
        ) or eletrico_generico_confiavel or tecnico_dominante_confiavel
        ...
        if 'tecnico_dominante_confiavel' in locals() and tecnico_dominante_confiavel and high_conditions:
            motivos.append("candidato tecnicamente dominante com consumo válido; match alto geral")
        if not dominante:
